...
- Only pass `--read-only` when the user explicitly requests read-only behavior. Treat `--read-only` and `--write` as runtime controls and do not include them in the task text.
